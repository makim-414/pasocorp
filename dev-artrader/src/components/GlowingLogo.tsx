    ...
    <div className="relative w-28 h-28 md:w-32 md:h-32">
      ...
      <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" style={{ transform: "scale(1.8)" }}>
        {sparkles.map((s, i) => {
          ...
          const size = 1 + 0.8 * Math.max(0, Math.sin(pulse * 2 + s.delay * Math.PI));
          return (
            <g key={i} opacity={opacity}>
              <line x1={s.cx - size * 2} y1={s.cy} x2={s.cx + size * 2} y2={s.cy} stroke="#4ade80" strokeWidth="0.5" />
              <line x1={s.cx} y1={s.cy - size * 2} x2={s.cx} y2={s.cy + size * 2} stroke="#4ade80" strokeWidth="0.5" />
              <circle cx={s.cx} cy={s.cy} r={size * 0.4} fill="#4ade80" />
            </g>
          );
        })}
      </svg>
