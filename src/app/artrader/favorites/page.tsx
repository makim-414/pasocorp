...
import { getArtistBySlug, artists, type Artist, type ArtistAuctionRecord } from "@/lib/artists";
...
export default function FavoritesPage() {
    ...
    for (const fav of raw) {
      ...
      if (artist && artist.recentAuctions[fav.auctionIndex]) {
        withData.push({
          ...fav,
          artist,
          auction: artist.recentAuctions[fav.auctionIndex],
        });
      }
    }
  ...
  // Price summary across all favorites
    ...
    <div className="max-w-[1400px] mx-auto px-6 md:px-12 py-12">
      ...
      {favorites.length === 0 ? (
        <div className="text-center py-20">
          ...
          <Link
            href="/artrader"
            className="inline-block px-6 py-3 bg-[#4ade80] text-black rounded-lg text-sm font-semibold hover:bg-[#3ecf73] transition-colors"
          >
            작가 검색하기
          </Link>
        </div>
            ...
            return (
              <div key={slug} className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-lg p-6 mb-6">
                <div className="flex items-start justify-between mb-4">
                  <Link href={`/artrader/artist/${slug}`} className="group">
                    ...
                    <div className="flex items-center gap-3 mt-1">
                      <span className="text-[10px] text-[#4ade80] bg-[#4ade80]/10 px-2 py-0.5 rounded">
                        {artist.category}
                      </span>
                ...
                <div className="space-y-3">
                  {items.map((fav) => (
                    <div
                      key={`${fav.artistSlug}-${fav.auctionIndex}`}
                      className="flex items-center justify-between bg-[#111] rounded-lg p-4 hover:bg-[#151515] transition-colors"
                    >
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-[#e8e8e8] font-medium truncate">{fav.auction.title}</p>
                        <p className="text-xs text-[#555] mt-0.5">
                          {fav.auction.medium} · {fav.auction.size} · {fav.auction.date}
                        </p>
                      </div>
                      <div className="flex items-center gap-4 shrink-0 ml-4">
                        ...
                        <button
                          onClick={() => handleRemove(fav.artistSlug, fav.auctionIndex)}
