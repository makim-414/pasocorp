...
export default function TrendingArtistsMarquee({ onArtistClick }: { onArtistClick?: (name: string) => void }) {
      ...
      >
        {doubled.map((name, i) => (
          <button
            ...
            className="inline-block px-5 py-2.5 border border-[#333] rounded-full text-sm text-white whitespace-nowrap hover:border-[#4ade80] hover:text-[#4ade80] transition-colors cursor-pointer bg-transparent"
          >
            {name}
          </button>
        ))}
