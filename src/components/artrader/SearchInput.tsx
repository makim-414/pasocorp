...
export default function SearchInput({ variant = "nav", initialQuery = "" }: SearchInputProps) {
  ...
  const handleChange = (value: string) => {
    ...
    if (q.length > 0) {
      ...
      // Also show all artists that partially match for better suggestions
  ...
  const navigateToArtist = (artist: Artist) => {
    ...
    router.push(`/artrader/artist/${artist.slug}`);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // If a suggestion is selected via keyboard, go to that artist
    if (selectedIndex >= 0 && selectedIndex < suggestions.length) {
      navigateToArtist(suggestions[selectedIndex]);
      return;
    }

    // If exact match (single result), go directly to artist page
    const results = searchArtists(q);
    if (results.length === 1) {
      router.push(`/artrader/artist/${results[0].slug}`);
    } else {
