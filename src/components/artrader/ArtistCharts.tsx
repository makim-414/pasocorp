...
/** Compute nice Y-axis bounds: zoom in when range is small relative to values */
function computeYBounds(values: number[]): { yMin: number; yMax: number; ticks: number[] } {
  ...
  if (range === 0) {
    // All same value — show ±10% band
    const pad = rawMin * 0.1 || 1;
    yMin = rawMin - pad;
    yMax = rawMax + pad;
  } else if (range / rawMax < 0.3) {
    // Values are close: zoom in — pad 20% of range on each side
    const pad = range * 0.3;
    yMin = rawMin - pad;
    yMax = rawMax + pad;
  } else {
    // Normal spread — start from 0
    yMin = 0;
    yMax = rawMax * 1.15;
  }
...
export default function ArtistCharts({ auctions, hammerRate }: ArtistChartsProps) {
  ...
  // Sort auctions by date (oldest first) for the price chart
  ...
  // Hammer rate chart data — simulate quarterly variation around the rate
              ...
              return (
                <g key={i}>
                  <rect
                    ...
                    height={barHeight}
                    rx="3"
                    fill={val >= rateNum ? "#4ade80" : "#4ade80"}
